...
#include "fake_robot_hardware/fake_robot_hardware.hpp"
...
FakeRobotHardware::init()
{
  ...
  js4 = hardware_interface::JointStateHandle(
    joint_name4, &pos4, &vel4, &eff4);
  ret = register_joint_state_handle(&js4);
  if (ret != hardware_interface::HW_RET_OK) {
    RCUTILS_LOG_WARN("can't register joint state handle %s", joint_name4.c_str());
    return ret;
  }

  js5 = hardware_interface::JointStateHandle(
    joint_name5, &pos5, &vel5, &eff5);
  ret = register_joint_state_handle(&js5);
  if (ret != hardware_interface::HW_RET_OK) {
    RCUTILS_LOG_WARN("can't register joint state handle %s", joint_name5.c_str());
    return ret;
  }

  js6 = hardware_interface::JointStateHandle(
    joint_name6, &pos6, &vel6, &eff6);
  ret = register_joint_state_handle(&js6);
  if (ret != hardware_interface::HW_RET_OK) {
    RCUTILS_LOG_WARN("can't register joint state handle %s", joint_name6.c_str());
    return ret;
  }
  ...
  jcmd4 = hardware_interface::JointCommandHandle(joint_name4, &cmd4);
  ret = register_joint_command_handle(&jcmd4);
  if (ret != hardware_interface::HW_RET_OK) {
    RCUTILS_LOG_WARN("can't register joint command handle %s", joint_name4.c_str());
    return ret;
  }

  jcmd5 = hardware_interface::JointCommandHandle(joint_name5, &cmd5);
  ret = register_joint_command_handle(&jcmd5);
  if (ret != hardware_interface::HW_RET_OK) {
    RCUTILS_LOG_WARN("can't register joint command handle %s", joint_name5.c_str());
    return ret;
  }

  jcmd6 = hardware_interface::JointCommandHandle(joint_name6, &cmd6);
  ret = register_joint_command_handle(&jcmd6);
  if (ret != hardware_interface::HW_RET_OK) {
    RCUTILS_LOG_WARN("can't register joint command handle %s", joint_name6.c_str());
    return ret;
  }
...
hardware_interface::hardware_interface_ret_t
FakeRobotHardware::read()
{
  return hardware_interface::HW_RET_OK;
}

hardware_interface::hardware_interface_ret_t
FakeRobotHardware::write()
{
  pos1 = cmd1;
  pos2 = cmd2;
  pos3 = cmd3;
